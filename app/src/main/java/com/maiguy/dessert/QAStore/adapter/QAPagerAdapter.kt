...
class QAPagerAdapter(
    val context: Context,
    private val choice: ArrayList<QAObject>,
    val dialog: Dialog,
    val viewpager: ViewPager2,
    val type: String
) : RecyclerView.Adapter<QAPagerAdapter.Holder?>() {
    ...
    private fun questionConfirm(chk1: Int, chk2: Int, position: Int) {
        ...
        if (chk1 == -1 || chk2 == -1) {
            Toast.makeText(
                context,
                context.getString(R.string.selected_questions),
                Toast.LENGTH_SHORT
            ).show()
        } else {
            when (chk1) {
            ...
            val inputMap = mapOf(
                "id" to choice[position].questionId,
                "question" to answerQA,
                "weight" to answerWeight
            )
    ...
    private fun feedbackFinish() {
        ...
        db.child("feedback_result").setValue(true)
        GlobalVariable.feedbackResult = false
    ...
    private fun questionFinish() {
        ...
        FirebaseDatabase.getInstance().reference.child("Users").child(userId).child("Questions")
            .updateChildren(hashMapQA)
        dialog.dismiss()
    }
